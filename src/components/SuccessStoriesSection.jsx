...
const SuccessStoriesSection = () => {
  return (
    <section
      id="stories"
      className="py-24 bg-[#0a0a0a] border-t border-neutral-900 overflow-hidden"
    >
          ...
          >
            {duplicatedStories.map((story, index) => (
              <motion.div
                ...
                className="w-70 sm:w-87.5 shrink-0 group relative aspect-4/5 rounded-4xl overflow-hidden border border-neutral-800/50"
              >
                <Image
                  src={story.image}
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-accent/40 rounded-4xl transition-colors duration-500 pointer-events-none" />
              </motion.div>
            ))}
          </motion.div>

          {/* Edge Fades */}
          <div className="absolute top-0 left-0 w-32 h-full bg-linear-to-r from-[#0a0a0a] to-transparent z-10" />
          <div className="absolute top-0 right-0 w-32 h-full bg-linear-to-l from-[#0a0a0a] to-transparent z-10" />
        </div>
      </div>
    </section>
  );
